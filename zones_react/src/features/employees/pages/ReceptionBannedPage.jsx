...
export default function ReceptionBannedPage() {
  ...
  useEffect(() => {
    window.addEventListener(CUSTOMER_BAN_EVENT, refresh);
    window.addEventListener("focus", refresh);
    const timer = setInterval(refresh, 60_000);
    return () => {
      window.removeEventListener(CUSTOMER_BAN_EVENT, refresh);
      window.removeEventListener("focus", refresh);
      clearInterval(timer);
    };
  }, [refresh]);

  const bans = useMemo(() => loadBanRecords(tick), [tick]);
  ...
  const handleLiftBan = async (row) => {
    ...
    const result = liftBanManually(row.id);
    if (!result.ok) {
